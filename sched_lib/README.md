...
3.```int get_task_status(scheduler_datastate_block_t* sptr, int task_type)``` - this routine is used to check the current status of a Task that has been submitrted via ```request_execution`` into the Scheuduler.  The list of Task status results is defined in the ```scheduler.h``` include file.
