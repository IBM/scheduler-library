...
void print_plan_ctrl_metadata_block_contents(void *mb_ptr) {
  ...
  printf("  PLAN_CTRL: in_veh_state  : Active %u Lane %u Speed %.1f\n",
         plan_ctrl_data_p->vehicle_state.active, plan_ctrl_data_p->vehicle_state.lane,
...
void execute_on_cpu_plan_ctrl_accelerator(void *task_metadata_block_ptr) {
  ...
  plan_ctrl_data_struct_t *plan_ctrl_data_p = (plan_ctrl_data_struct_t *)(
  ...
  DEBUG(printf("In the plan_and_control task : label %u %s distance %.1f (T1 %.1f T2 %.1f T3 %.1f) message %u\n",
               plan_ctrl_data_p->object_label, object_names[plan_ctrl_data_p->object_label],
               plan_ctrl_data_p->object_distance,
    ...
    if (plan_ctrl_data_p->object_distance <= IMPACT_DISTANCE) {
      ...
      case unsafe_to_move_left_or_right:
        ...
        if (plan_ctrl_data_p->vehicle_state.speed > car_decel_rate) {
          plan_ctrl_data_p->new_vehicle_state.speed = plan_ctrl_data_p->vehicle_state.speed -
...
void *set_up_plan_ctrl_task(void *sptr_ptr, task_type_t plan_ctrl_task_type,
  ...
  */
  ...
  DEBUG(printf("   Set MB%u time_step %u rpt_fac %u obj %u dist %.1f msg %u VS : act %u lane %u Spd %.1f \n",
               plan_ctrl_mb_ptr->block_id, plan_ctrl_data_p->time_step, plan_ctrl_data_p->repeat_factor,
               plan_ctrl_data_p->object_label,
               plan_ctrl_data_p->object_distance, plan_ctrl_data_p->safe_lanes_msg,
...
void finish_plan_ctrl_execution(void *plan_ctrl_metadata_block_ptr, void *args) {
  ...
  plan_ctrl_timing_data_t *plan_ctrl_timings_p = (plan_ctrl_timing_data_t *) &
      (plan_ctrl_metadata_block->task_timings[plan_ctrl_metadata_block->task_type]);
  plan_ctrl_data_struct_t *plan_ctrl_data_p = (plan_ctrl_data_struct_t *)(
        plan_ctrl_metadata_block->data_space);
#ifdef INT_TIME
  struct timeval stop_time;
  gettimeofday(&stop_time, NULL);
