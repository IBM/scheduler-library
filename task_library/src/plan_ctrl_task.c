...
void print_plan_ctrl_metadata_block_contents(task_metadata_block_t *mb) {
  ...
  plan_ctrl_data_struct_t *plan_ctrl_data_p =
      (plan_ctrl_data_struct_t *)(mb->data_space);
  ...
  printf("  PLAN_CTRL: in_veh_state  : Active %u Lane %u Speed %.1f\n",
         ...
         plan_ctrl_data_p->vehicle_state.lane,
         plan_ctrl_data_p->vehicle_state.speed);
  printf("  PLAN_CTRL: out_veh_state : Active %u Lane %u Speed %.1f\n",
         plan_ctrl_data_p->new_vehicle_state.active,
         plan_ctrl_data_p->new_vehicle_state.lane,
         plan_ctrl_data_p->new_vehicle_state.speed);
}

void output_plan_ctrl_task_type_run_stats(scheduler_datastate_block_t *sptr,
...
void execute_on_cpu_plan_ctrl_accelerator(
  ...
  plan_ctrl_data_struct_t *plan_ctrl_data_p =
      (plan_ctrl_data_struct_t *)(task_metadata_block->data_space);
  ...
  DEBUG(printf("In the plan_and_control task : label %u %s distance %.1f (T1 "
  ...
  plan_ctrl_timings_p->call_sec[aidx] +=
      stop_time.tv_sec - plan_ctrl_timings_p->call_start.tv_sec;
  plan_ctrl_timings_p->call_usec[aidx] +=
...
task_metadata_block_t *set_up_plan_ctrl_task(
  ...
  DEBUG(
      printf("Calling get_task_metadata_block for Critical PLAN_CTRL-Task %u\n",
             plan_ctrl_task_type));
  do {
    plan_ctrl_mb_ptr = get_task_metadata_block(
        sptr, dag_id, plan_ctrl_task_type, crit_level, plan_ctrl_profile);
