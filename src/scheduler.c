...
vitHW_token_t *vitHW_lmem[MAX_NUM_VIT_ACCEL];   // Pointer to local view of contig memory
vitHW_token_t *vitHW_li_mem[MAX_NUM_VIT_ACCEL]; // Pointer to input memory block
vitHW_token_t *vitHW_lo_mem[MAX_NUM_VIT_ACCEL]; // Pointer to output memory block
size_t vitHW_in_len[MAX_NUM_VIT_ACCEL];
size_t vitHW_out_len[MAX_NUM_VIT_ACCEL];
size_t vitHW_in_size[MAX_NUM_VIT_ACCEL];
size_t vitHW_out_size[MAX_NUM_VIT_ACCEL];
size_t vitHW_out_offset[MAX_NUM_VIT_ACCEL];
size_t vitHW_size[MAX_NUM_VIT_ACCEL];

struct vitdodec_access vitHW_desc[MAX_NUM_VIT_ACCEL];
...
fftHW_token_t* fftHW_lmem[MAX_NUM_FFT_ACCEL];  // Pointer to local version (mapping) of fftHW_mem
fftHW_token_t* fftHW_li_mem[MAX_NUM_FFT_ACCEL]; // Pointer to input memory block
fftHW_token_t* fftHW_lo_mem[MAX_NUM_FFT_ACCEL]; // Pointer to output memory block
size_t fftHW_in_len[MAX_NUM_FFT_ACCEL];
size_t fftHW_out_len[MAX_NUM_FFT_ACCEL];
size_t fftHW_in_size[MAX_NUM_FFT_ACCEL];
size_t fftHW_out_size[MAX_NUM_FFT_ACCEL];
size_t fftHW_out_offset[MAX_NUM_FFT_ACCEL];
size_t fftHW_size[MAX_NUM_FFT_ACCEL];
struct fftHW_access fftHW_desc[MAX_NUM_FFT_ACCEL];
...
execute_task_on_accelerator(task_metadata_block_t* task_metadata_block)
{
  ...
  case sm_fft_hwr_accel_t: {
    DEBUG(printf("Executing Task for MB%d on SMALL HWR_FFT_ACCELERATOR\n", task_metadata_block->block_id));
    execute_hwr_fft_accelerator(task_metadata_block);
  } break;
  ...
  case sm_vit_hwr_accel_t: {
    DEBUG(printf("Executing Task for MB%d on SMALL HWR_VITERBI_ACCELERATOR\n", task_metadata_block->block_id));
    execute_hwr_viterbi_accelerator(task_metadata_block);
  } break;
  case cv_hwr_accel_t: {
  ...
  case sm_cv_hwr_accel_t: {
    DEBUG(printf("Executing Task for MB%d on SMALL HWR_CV_ACCELERATOR\n", task_metadata_block->block_id));
    execute_hwr_cv_accelerator(task_metadata_block);
  } break;
  default:
...
status_t initialize_scheduler()
{
  ...
  int parms_error = 0;
  if ((NUM_FFT_ACCEL + NUM_SM_FFT_ACCEL) > MAX_NUM_FFT_ACCEL) {
	  printf("INIT-SCHED: ERROR : (NUM_FFT_ACCEL + NUM_SM_FFT_ACCEL) > MAX_NUM_FFT_ACCEL : %u + %u > %u\n", NUM_FFT_ACCEL, NUM_SM_FFT_ACCEL, MAX_NUM_FFT_ACCEL);
	  parms_error = 1;
  }
  if ((NUM_VIT_ACCEL + NUM_SM_VIT_ACCEL) > MAX_NUM_VIT_ACCEL) {
	  printf("INIT-SCHED: ERROR : (NUM_VIT_ACCEL + NUM_SM_VIT_ACCEL) > MAX_NUM_VIT_ACCEL : %u + %u > %u\n", NUM_VIT_ACCEL, NUM_SM_VIT_ACCEL, MAX_NUM_VIT_ACCEL);
	  parms_error = 1;
  }
  if ((NUM_CV_ACCEL + NUM_SM_CV_ACCEL) > MAX_NUM_CV_ACCEL) {
	  printf("INIT-SCHED: ERROR : (NUM_CV_ACCEL + NUM_SM_CV_ACCEL) > MAX_NUM_CV_ACCEL : %u + %u > %u\n", NUM_CV_ACCEL, NUM_SM_CV_ACCEL, MAX_NUM_CV_ACCEL);
	  parms_error = 1;
  }
  if (parms_error > 0) {
    printf("... Exiting run...\n");
  ...
  for (int i = 0; i < MAX_ACCEL_OF_EACH_TYPE+1; i++) {
    for (int j = 0; j < MAX_NUM_FFT_ACCEL+1; j++) {
      for (int sj = 0; sj < MAX_NUM_SM_FFT_ACCEL+1; sj++) {
	for (int k = 0; k < MAX_NUM_VIT_ACCEL+1; k++) {
	  for (int sk = 0; sk < MAX_NUM_SM_VIT_ACCEL+1; sk++) {
	    for (int l = 0; l < MAX_NUM_CV_ACCEL+1; l++) {
	      for (int sl = 0; sl < MAX_NUM_SM_CV_ACCEL+1; sl++) {
...
fastest_to_slowest_first_available(ready_mb_task_queue_entry_t* ready_task_entry)
{
  ...
  switch(task_metadata_block->job_type) {
  case FFT_TASK: {
    // Scheduler should now run this either on CPU or FFT or SM-FFT:
    do {
      int i = 0;
#ifdef HW_FFT
      proposed_accel = fft_hwr_accel_t;
...
fastest_finish_time_first(ready_mb_task_queue_entry_t* ready_task_entry)
{
  ...
  case FFT_TASK: {   // Scheduler should run this either on CPU or FFT
    ...
    DEBUG(printf("SCHED_FFF:    Set prop_acc[%u] = %u = %s  with %u FFT and %u SM-FFT\n", (num_proposed_accel_types-1), proposed_accel[num_proposed_accel_types-1], accel_type_str[proposed_accel[num_proposed_accel_types-1]], NUM_FFT_ACCEL, NUM_SM_FFT_ACCEL));
//#ifdef HW_FFT
...
//#endif
  ...
  case VITERBI_TASK: {  // Scheduler should run this either on CPU or VIT
    proposed_accel[num_proposed_accel_types++] = cpu_accel_t;
    DEBUG(printf("SCHED_FFF:    Set prop_acc[%u] = %u = %s  with %u VIT and %u SM-VIT\n", (num_proposed_accel_types-1), proposed_accel[num_proposed_accel_types-1], accel_type_str[proposed_accel[num_proposed_accel_types-1]], NUM_VIT_ACCEL, NUM_SM_VIT_ACCEL));
#ifdef HW_VIT
    DEBUG(printf("SCHED_FFF:     Have HW_VIT : NUM_VIT_ACCEL = %u and NUM_SM_VIT_ACCEL = %u\n", NUM_VIT_ACCEL, NUM_SM_VIT_ACCEL));
    if (NUM_VIT_ACCEL > 0) {
  ...
  char* ti_label[2] = {"CPU", "HWR"};
  {
    ...
    for (int ti = 0; ti < 2; ti++) {
      for (int bi = 0; bi < total_metadata_pool_blocks; bi++) {
        ...
        uint64_t this_call_usec = (uint64_t)(master_metadata_pool[bi].fft_timings.call_sec[ti]) * 1000000 + (uint64_t)(master_metadata_pool[bi].fft_timings.call_usec[ti]);
        ...
        printf("Block %3u : %u %s : FFT CB %8u call %15lu fft %15lu fft_br %15lu br %15lu cvtin %15lu calc %15lu cvto %15lu fmcw %15lu usec\n", bi, ti, ti_label[ti], this_comp_by, this_call_usec, this_fft_usec, this_fft_br_usec, this_bitrev_usec, this_fft_cvtin_usec, this_fft_comp_usec, this_fft_cvtout_usec, this_cdfmcw_usec);
    ...
    avg0 = (double)total_call_usec[0] / (double) freed_metadata_blocks[FFT_TASK];
    avg1 = (double)total_call_usec[1] / (double) freed_metadata_blocks[FFT_TASK];
    avg2 = (double)(total_call_usec[0] + total_call_usec[1]) / (double) freed_metadata_blocks[FFT_TASK];
    printf("     fft-call run time   %u %s %8u %15lu usec %16.3lf avg : %u %s %8u %15lu usec %16.3lf avg : TOT %8u %15lu usec %16.3lf avg\n", 0, ti_label[0], total_fft_comp_by[0], total_call_usec[0], avg0, 1, ti_label[1], total_fft_comp_by[1], total_call_usec[1], avg1, total_fft_comp_by[2], total_call_usec[2], avg2);
    ...
    //uint64_t total_call_usec[3] = {0, 0, 0}; // re-use the FFT one by same name...
    ...
    for (int ti = 0; ti < 2; ti++) {
      for (int bi = 0; bi < total_metadata_pool_blocks; bi++) {
        ...
        printf("Block %3u : %u %s : CV %8u call-time %15lu parse %15lu usec\n", bi, ti, ti_label[ti], this_comp_by, this_call_usec, this_parse_usec);
        // Per acceleration (CPU, HWR)
    ...
    printf("     CNN-call  run time   %u %s %8u %15lu usec %16.3lf avg : %u %s %8u %15lu usec %16.3lf avg : TOT %8u %15lu usec %16.3lf avg\n", 0, ti_label[0], total_cv_comp_by[0], total_call_usec[0], avg0, 1, ti_label[1], total_cv_comp_by[1], total_call_usec[1], avg1, total_cv_comp_by[2], total_call_usec[2], avg2);
