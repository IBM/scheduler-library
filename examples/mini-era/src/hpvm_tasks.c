...
void
__attribute__((noinline))
pnc_leaf(uint32_t time_step, uint32_t repeat_factor, label_t * obj_label,
  size_t obj_label_size, distance_t * distance_ptr, size_t distance_ptr_size,
  message_t * message_id, size_t msg_id_size, vehicle_state_t * current_vehicle_state,
  size_t current_vehicle_state_size, vehicle_state_t * new_vehicle_state,
  size_t new_vehicle_state_size, char * out_msg_text, size_t out_msg_text_size) {
...
void MiniERARoot(
  ...
  void * Section = __hetero_section_begin();
  {
    ...
    void * RADAR_POST = __hetero_task_begin(4, (size_t) 0, log_nsamples, inputs_ptr, inputs_ptr_size, distance_ptr, distance_ptr_size, 1, distance_ptr, distance_ptr_size,
    ...
    void * PNC = __hetero_task_begin(9, (size_t) 0, time_step, repeat_factor, obj_label, obj_label_size, distance_ptr, distance_ptr_size, message_id, msg_id_size, current_vehicle_state, current_vehicle_state_size, out_msg_text, out_msg_text_size, new_vehicle_state, new_vehicle_state_size, 1, new_vehicle_state, new_vehicle_state_size,
      /* Optional Node Name */ "PNC");

    // Body will be inlined into task
    pnc_leaf(time_step, repeat_factor, obj_label, obj_label_size, distance_ptr, distance_ptr_size, message_id, msg_id_size, current_vehicle_state, current_vehicle_state_size, new_vehicle_state, new_vehicle_state_size, out_msg_text, out_msg_text_size);
...
void hpvm_launch(message_size_t vit_msgs_size, vit_dict_entry_t * vdentry_p, uint8_t * vit_depunctured, message_t * message, char * out_msg_text, uint8_t * vit_data, label_t * cv_tr_label, unsigned log_nsamples, float * radar_inputs, distance_t * distance, unsigned time_step, unsigned pandc_repeat_factor, vehicle_state_t * vehicle_state, vehicle_state_t * new_vehicle_state) {
  ...
  size_t fft_size = (size_t) log_nsamples;
  size_t cv_size = (size_t) sizeof(label_t);
...
extern "C" {
  //TODO: Remove this and makesure it is being called in the backend
  void hpvm_initialize() {
    __hpvm__init();
  }
}
...
void hpvm_launch_base_VIT(message_size_t vit_msgs_size, vit_dict_entry_t * vdentry_p, uint8_t * vit_depunctured, message_t * message, char * out_msg_text, uint8_t * vit_data) {

  size_t vit_size = vit_msgs_size;
  void * DFG = __hetero_launch((void *) VITRoot,
    /* Num Input Pairs */ 8,
    vit_size,
    vit_msgs_size,
    &(vit_data[0]), sizeof(vit_data),
...
void RadarRoot(
  ...
  void * Section = __hetero_section_begin();
  {
    ...
    void * RADAR = __hetero_task_begin(3, in_size, log_nsamples, inputs_ptr, inputs_ptr_size, 1, inputs_ptr, inputs_ptr_size,
      /* Optional Node Name */ "RADAR");
    // printf("In RADAR_LEAF\n");
    // Body will be inlined into task
    radar_leaf(in_size, log_nsamples, inputs_ptr, inputs_ptr_size);
...
void hpvm_launch_base_RADAR(unsigned log_nsamples, float * radar_inputs, distance_t * distance) {
  void * DFG = __hetero_launch((void *) RadarRoot,
    /* Num Input Pairs */ 3,
    log_nsamples,
    radar_inputs, radar_inputs_size,
    distance, sizeof(distance_t),
...
void CVRoot(
  ...
  void * Section = __hetero_section_begin();
  {
    void * CV = __hetero_task_begin(3, (size_t) sizeof(label_t), in_label, obj_label, obj_label_size, 1, obj_label,
      obj_label_size,
      /* Optional Node Name */ "CV");

    // Body will be inlined into task
    cv_leaf((size_t) sizeof(label_t), in_label, obj_label, obj_label_size);
...
void hpvm_launch_base_CV(label_t cv_tr_label) {
  void * DFG = __hetero_launch((void *) CVRoot,
    /* Num Input Pairs */ 2,
    *cv_tr_label,
    cv_tr_label, sizeof(label_t),
    /* Num Output Pairs */ 1,
