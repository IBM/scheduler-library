...
screen_width = 825
screen_height = 500
lane_width = screen_width / 11 # 9 lanes + 2 grass sides
road_width = lane_width * 9 # 9 lanes
...
x_main_car = x_left # your car's x position, default shouldn't matter...
...
nine_lane_trace = True;


# HELPER FUNCTIONS
def set_background(nine_lane_trace):
    """
    ...
    pygame.draw.rect(screen, LIGHT_GREY, pygame.Rect(2*lane_width, 0, 7*lane_width, screen_height)) # road
    ...
    pygame.draw.line(screen, TANGERINE, [lane_width*8, 0], [lane_width*8, screen_height], 3) # seventh road line
    pygame.draw.line(screen, TANGERINE, [lane_width*9, 0], [lane_width*9, screen_height], 3) # eighth road line
    #screen.blit(get_img('images/trees_bg1.png'), (-20, bgY)) # scrolling grass bg
    #screen.blit(get_img('images/trees_bg1.png'), (-20, bgY2))
    screen.blits(((get_img('images/trees_bg1.png'), (-20, bgY)),  (get_img('images/trees_bg1.png'), (lane_width*10, bgY)),
                  (get_img('images/trees_bg1.png'), (-20, bgY2)), (get_img('images/trees_bg1.png'), (lane_width*10, bgY2))), False)
...
def parse_trace(filename, nine_lanes):  
    """
    ...
    for line in lines:
        ...
        if nine_lanes :
            ...
            far_l_lane.append(tokens[2])
            left_lane.append( tokens[3])
            mleft_lane.append(tokens[4])
            mid_lane.append(  tokens[5])
            mrght_lane.append(tokens[6])
            right_lane.append(tokens[7])
            far_r_lane.append(tokens[8])
            rhaz_lane.append( tokens[9])
            other_lane.append(tokens[10].strip("\n"))
        else:
            far_l_lane.append(tokens[1])
            ...
            far_r_lane.append(tokens[7])
            other_lane.append(tokens[8].strip("\n"))

    return my_lane, lhaz_lane, far_l_lane, left_lane, mleft_lane, mid_lane,mrght_lane, right_lane, far_r_lane, rhaz_lane, other_lane
...
def usage_and_exit(exit_code):
    ...
    print("          -7      or --nine--lane : indicates the trace has obstacles in all nine lanes");
...
def main(argv):
    ...
    global x_lhaz, x_far_l, x_left, x_mid, x_right, x_far_r, x_rhaz, x_main_car, y_main_car, x_other_car, y_other_car
    global MOVE_DOWN
    global obj_list
    global nine_lane_trace
    ...
    for i in range(0, len(argv[1:])):
        ...
        if ((argv[ii] == "-h") | (argv[ii] == "--help")) :
            ...
            nine_lane_trace = False;
            print("%s using Nine-Lane trace\n" % (argv[0]));
        elif ((argv[ii] == "-9") | (argv[ii] == "--nine-lane")) :
            nine_lane_trace = True;
            print("%s using Nine-Lane trace\n" % (argv[0]));
    ...
    print("%s using Nine-Lane trace = %u\n" % (argv[0], nine_lane_trace));
    ...
    mine, lhaz, far_l, left, mid_l, mid, mid_r, right, far_r, rhaz, other  = parse_trace(tracefile, nine_lane_trace)
    print("Sizes of mine %u lhaz %u far_l %u left %u mid %u right %u far_r %u rhaz %u\n" % (len(mine), len(lhaz), len(far_l), len(left), len(mid), len(right), len(far_r), len(rhaz)))
    ...
    x_per_lane = (x_lhaz, x_far_l, x_left, x_mid_l, x_mid, x_mid_r, x_right, x_far_r, x_rhaz)
    ...
    while not done:
        for event in pygame.event.get():
            ...
            if event.type == move_down_event:
                ...
                lobjs = far_l_data.split(" ")
                for obj in lobjs:
                    (trtype, trdist) = obj.split(":")
                    trtup  = (x_far_l, 500 - get_dist(trdist), trtype) # get_object(trtype))
                    obj_list.append(trtup);
                ...
                robjs = far_r_data.split(" ")
                for obj in robjs:
                    (trtype, trdist) = obj.split(":")
                    trtup  = (x_far_r, 500 - get_dist(trdist), trtype) #get_object(trtype))
                    obj_list.append(trtup);
        ...
        # Set background
        set_background(nine_lane_trace)
