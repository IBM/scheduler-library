...
assign_task_to_pe(scheduler_datastate* sptr) {
  ...
  for (auto task_entry : ready_queue) {
    ...
    if ((accel_type == NO_Accelerator) || (accel_id == -1)) {
      ...
      for (int check_accel = sptr->next_avail_accel_id - 1; check_accel >= 0;
           check_accel--) { // Last accel is "no-accelerator"
        ...
        if (sptr->scheduler_execute_task_function[check_accel][task_metadata_block->task_type] != NULL) {
          ...
          while ((i < sptr->num_accelerators_of_type[check_accel])) { // && (accel_id < 0)) {
            ...
            if (bi == -1) { // Not in use -- available
              ...
              int64_t remaining_time = sptr->master_metadata_pool[bi].task_on_accel_profile[check_accel] -
                                       total_elapsed_usec;
